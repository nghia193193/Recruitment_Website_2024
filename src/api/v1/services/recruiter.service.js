...
class RecruiterService {
  static signUp = async ({
    ...
    try {
      ...
      if (recruiter) {
        ...
        if (otpHolder.length) {
          throw new BadRequestError(
            "Email này đã được đăng ký, vui lòng truy cập email để xác nhận tài khoản"
          );
        }
      ...
      await EmailService.sendSignUpMail({
        toEmail: email,
        userName: newRecruiter[0].name,
        code: mapRolePermission["RECRUITER"],
      });
  ...
  static verifyEmail = async (email, otp) => {
    ...
    try {
      ...
      if (!hashPassword) {
        throw new BadRequestError(
          "Quá thời hạn 24 giờ kể từ lúc đăng ký vui lòng đăng ký lại."
        );
      }
  ...
  static getInformation = async ({ userId }) => {
    try {
      const recruiterInfor = await Recruiter.findById(userId)
        .populate("loginId")
        .lean()
      ...
      const recruiterPostLimit = await RecruiterPostLimit.findOne({
        recruiterId: userId,
      }).lean();
      const premiumAccount = await Order.checkPremiumAccount({
        recruiterId: userId,
      });
      const recruiterBannedJobCount =
        await JobService.getRecruiterBannedJobCount({ recruiterId: userId });
      let limitPost;
      if (premiumAccount) {
      ...
      const likeNumber = await FavoriteRecruiterService.getLikeNumber({
        recruiterId: userId,
      });
      ...
      recruiterInfor.companyCoverPhoto =
        recruiterInfor.companyCoverPhoto ?? null;
  ...
  static updateAvatar = async ({ userId, avatar }) => {
    try {
      ...
      const result = await Recruiter.findById(userId)
        .populate("loginId")
        .select("-__v")
        .lean();
      result.role = result.loginId.role;
  ...
  static updateProfile = async ({
    ...
    try {
      const recruiter = await Recruiter.findById(userId)
        .select(
          "-__v -_id -loginId -verifyEmail -firstApproval -firstUpdate -createdAt -updatedAt -acceptanceStatus"
        )
        .lean();
      let result;
  ...
  static updateCompany = async ({
    ...
    try {
      ...
      let result;
      if (!recruiter.oldInfo?.name) {
        result = await Recruiter.findByIdAndUpdate(
          userId,
          {
            $set: {
              companyName,
        ...
        if (companyLogo) {
          if (
            ...
            recruiter.companyLogo !== recruiter.oldInfo.companyLogo
          ) {
            const splitArr = companyLogo.split("/");
            const image = splitArr[splitArr.length - 1];
            clearImage(image);
        ...
        if (companyCoverPhoto) {
          if (
            ...
            recruiter.companyCoverPhoto !== recruiter.oldInfo.companyCoverPhoto
          ) {
            const splitArr = companyCoverPhoto.split("/");
            const image = splitArr[splitArr.length - 1];
            clearImage(image);
  ...
  static changePassword = async ({ userId, currentPassword, newPassword }) => {
    try {
      ...
      const { message } = await Login.changePassword({
        email,
        currentPassword,
        newPassword,
      });
  ...
  static getInformationBySlug = async ({ slug }) => {
    try {
      let recruiterInfor = await Recruiter.findOne({ slug })
        .lean()
        .select(
      ...
      const likeNumber = await FavoriteRecruiterService.getLikeNumber({
        recruiterId: recruiterInfor._id.toString(),
      });
  ...
  static getListRelatedRecruiter = async ({
    recruiterId,
    searchText,
    page,
    limit,
  ...
  static createJob = async ({
    ...
    try {
      ...
      // Check limit post
      const recruiterLimitPost = await RecruiterPostLimit.findOne({
        recruiterId: userId,
      });
      if (recruiterLimitPost.postCount >= limitPost) {
        throw new BadRequestError(
          `Bạn đã đạt giới hạn số bài đăng tuyển dụng trong tháng là: ${limitPost} bài.`
        );
      }
      ...
      const isUpdate = await RecruiterPostLimit.findOneAndUpdate(
        { recruiterId: userId },
        {
          $inc: { postCount: 1 },
        },
      ...
      const listCandidate = await FavoriteRecruiter.find({
        favoriteRecruiters: userId.toString(),
      }).lean();
      ...
      if (listCandidate.length !== 0) {
        for (let i = 0; i < listCandidate.length; i++) {
          ...
          const socketId = userSockets.get(
            listCandidate[i].candidateId.toString()
          );
          if (socketId) {
            _io.to(socketId).emit("user_notification", notificationCandidate);
            console.log(
              `Notification sent to user ${listCandidate[i].candidateId}: ${notificationCandidate}`
            );
          }
        }
      }
  ...
  static approveApplication = async ({
    ...
    try {
      ...
      const { result, jobId, jobName, quantity } =
      ...
      if (status === "Đã nhận") {
        if (quantity !== "o") {
          if (acceptedNumber >= +quantity) {
            throw new BadRequestError(
              "Đã đủ số lượng cần tuyển, không thể nhận thêm!"
            );
          }
        }
      ...
      await EmailService.sendApplicationResultMail({
      ...
      if (socketId) {
        ...
        console.log(
          `Notification sent to user ${application.candidateId.toString()}: ${notification}`
        );
      }
      return {
