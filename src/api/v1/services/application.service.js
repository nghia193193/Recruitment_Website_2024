...
class ApplicationService {
  static getJobApplicationNumber = async function ({ jobId }) {
    try {
      ...
      const totalDocument = await Application.aggregate([
        ...pipeline,
  ...
  static getJobAcceptedApplicationNumber = async function ({ jobId }) {
    try {
      const totalElement = await Application.find({ jobId, status: "Đã nhận" })
        .lean()
        .countDocuments();
      return totalElement;
    } catch (error) {
  ...
  static getListJobApplication = async function ({
    userId,
    jobId,
    candidateName,
    experience,
    ...
    try {
      ...
      if (candidateName) {
        pipeline.push({
          $match: { "resume.name": new RegExp(candidateName, "i") },
        });
      }
      if (experience) {
        pipeline.push({
          $match: { "resume.experience": new RegExp(experience, "i") },
        });
      }
      if (status) {
      ...
      listApplication = listApplication.map((item) => {
        ...
        item.resume.updatedAt = formatInTimeZone(
          item.resume.updatedAt,
          "Asia/Ho_Chi_Minh",
          "dd/MM/yyy HH:mm:ss"
        );
        return {
      ...
      const acceptedNumber = await this.getJobAcceptedApplicationNumber({
        jobId,
      });
      return {
  ...
  static getApplicationDetail = async function ({ applicationId }) {
    try {
      ...
      result.createdAt = formatInTimeZone(
        result.createdAt,
        "Asia/Ho_Chi_Minh",
        "dd/MM/yyy HH:mm:ss"
      );
      result.updatedAt = formatInTimeZone(
        result.updatedAt,
        "Asia/Ho_Chi_Minh",
        "dd/MM/yyy HH:mm:ss"
      );
